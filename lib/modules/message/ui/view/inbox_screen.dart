...
class InboxScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final chatListController = Get.find<ActiveRideAndChatsController>();
    return Scaffold(
      ...
      body: Obx(()=>
        ListView.separated(
          itemCount: chatListController.rideChat.length,
          padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
          separatorBuilder: (context, index) =>
              Divider(height: 1, color: Colors.grey.shade300),
          itemBuilder: (context, index) {
            final participants = chatListController.rideChat[index].participants;
            return InkWell(
              onTap: () {
                Navigator.push(
                  context,
                  ...
                  children: [
                    ...
                    Column(
                      children: [
                        SizedBox(
                          height: 44,
                          width: (participants.length - 1) * 16 + 64,
                          child: Stack(
